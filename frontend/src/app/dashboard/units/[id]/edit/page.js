...
export default function EditUnitPage() {
  ...
  useEffect(() => {
    async function loadUnit() {
      try {
        ...
        setApiError(err.message || "Failed to load unit");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      await unitService.updateUnit(unitId, form);
      router.push("/dashboard/units");
    } catch (err) {
      setApiError(err.message || "Unable to update unit. Please try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className={styles.page}>
      <div>
        <h1>Edit Unit</h1>
        <p style={{ color: "#667085" }}>Update unit details</p>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={styles.formField}>
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="Describe this unit's purpose"
