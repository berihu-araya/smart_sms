...
export default function ClassTeachersListPage() {
  ...
  useEffect(() => {
    async function loadAssignments() {
      try {
        ...
        setError(err.message || "Failed to load class teachers");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleDeactivate = async (id, teacherName, sectionName) => {
    ...
    try {
      ...
      showToast(`Deactivated ${teacherName}`);
  ...
  if (loading) {
    return <div className={styles.loading}>Loading class teachers...</div>;
  }
    ...
    <div className={styles.page}>
      <div className={styles.headerRow}>
        <div className={styles.titleArea}>
          <h1>Class Teachers (Homeroom)</h1>
          <p>Assign and manage homeroom teachers for each class/section</p>
        </div>
        <Link href="/dashboard/class-teachers/new" className={styles.btnPrimary}>
          <HiPlus size={18} /> Assign Class Teacher
        </Link>
      </div>
      ...
      {filteredAssignments.length === 0 ? (
        <div className={styles.emptyState}>
          <p>No class teacher assignments found.</p>
          <Link href="/dashboard/class-teachers/new" className={styles.btnPrimary}>
            <HiPlus size={16} /> Assign First Class Teacher
          </Link>
        </div>
      ) : (
                    ...
                    <td>
                      <div className={styles.actionsCell} style={{ justifyContent: "flex-end" }}>
                        <button
                          ...
                          onClick={() => handleDeactivate(a.id, a.teacher_name, a.section_name)}
