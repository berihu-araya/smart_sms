...
export default function EditSubjectGroupPage() {
  ...
  useEffect(() => {
    async function loadGroup() {
      try {
        setLoading(true);
        const data = await subjectGroupService.getGroupById(params.id);
        setForm({
          groupName: data.group_name || "",
          description: data.description || "",
        });
      } catch (err) {
        setApiError(err.message || "Unable to load group");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setApiError(err.message || "Unable to update group. Please try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  if (loading) {
    return <div className={styles.page}>Loading group data...</div>;
  }
  ...
  return (
    <div className={styles.page}>
      ...
      <nav className={styles.breadcrumb}>
        ...
        <span className={styles.breadcrumbSeparator}>/</span>
        <span style={{ color: "#101828", fontWeight: 500 }}>Edit Group</span>
      </nav>

      {/* Page Header */}
      <div className={styles.pageHeader}>
        <h1>Edit Subject Group</h1>
        <p>Update the subject group information.</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className={styles.formCard}>
          <div className={styles.section}>
            <div className={styles.sectionHeader}>
              ...
              <div>
                ...
                <p className={styles.sectionSubtitle}>Update the subject group information</p>
              </div>
            </div>
          ...
          <div className={styles.actions}>
            <Link href={`/dashboard/subjects/groups/${params.id}`} className={styles.btnSecondary}>
              Cancel
            </Link>
